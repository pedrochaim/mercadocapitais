...
Nesta apresentação eu uso os termos "taxa de juros", "taxa de retorno", e "taxa de desconto" de forma intercambiável. Todos esses conceitos se referem ao $r$ em [@eq-juros], mas são ligeiramente distintos, sendo apropriados em contextos diferentes. 

Juros são o pagamento que o tomador faz ao credor pelo direito de usufruir do principal por um período de tempo pré-estabelecido em um empréstimo. Só faz sentido falar em taxa de juros quando existe um contrato entre duas partes e a transferência de recursos do emissor ao detentor é bem definida no momento que é firmado o contrato. Chamamos esse tipo de contrato de dívida de ativos de renda fixa. A quantia paga ao detentor do ativo pode ser completamente definida no momento da emissão (ativos prefixados) ou pode depender da realização de um índice amplamente acompanhado (ativos pós-fixados).

Em casos nos quais o ativo é uma cota de propriedade em uma empresa, como por exemplo no caso de ações de empresas de capital aberto, não existe um compromisso explícito de remuneração do capital empenhado. Como o detentor é parte dono da empresa, a remuneração do capital depende do resultado operacional dos investimentos e das estratégias de crescimento da empresa. Dizemos que são ativos de renda variável. Nesse caso não faz sentido falar em taxa de juros (ninguém te deve dinheiro), então falamos em taxa de retorno, que é um conceito mais genérico.

Quando falamos em taxa de desconto estamos aludindo ao custo de oportunidade de realizar certo investimento. É relacionado ao retorno que obteríamos com o melhor projeto de investimento alternativo (com mesmo perfil de risco).
...
Nossos exemplos até agora trataram de situações nas quais a operação acontece em dois períodos. Em $t=0$ pagamos uma quantia $S_0$ e em $t=1$ recebemos outra quantia $S_1$ (ou vice e versa). A taxa de juros é determinada pela diferença entre esses valores, seguindo a [@eq-eqvp]. A *frequência de incidência* da taxa de juros é definida pelo passo temporal entre $t=0$ e $t=1$.  
...
$$\begin{align*} S_1 &= S_0(1+r),\\  
  S_1&=\$1000(1+0.1)=\$1100.  
\end{align*}$$

Tomamos $S_0=\$1000$ em $t=0$, pagando uma taxa de $r=10\%$ ao ano, tal que nosso saldo devedor ao final do ano é $S_1=\$1100$. No início do ano $t=1$ o empréstimo é renovado, mas o "novo principal" inclui os juros devidos pelo período anterior.

$$\begin{align*} 
  S_2&=S_1(1+r)=S_0(1+r)(1+r)\\
  S_2&=S_0(1+r)^2,\\
  S_2&=\$1100(1+0.1)=\$1000(1+0.1)^2,\\
  S_2&=\$1210.
\end{align*}$$
...
A menos que explicitamente mencionado, consideraremos todas as operações de desconto e capitalização no regime composto.
...
A tabela abaixo descreve as taxas de juro realizadas de um certo ativo financeiro em quatro anos.
...
Qual é a taxa de juros média nesses quatro anos?

Uma solução aparente seria calcular a média aritmética das taxas de juro 

$$\begin{align}
\frac{10\%+8\%+7.5\%+13\%}{4}=9.625\%
\end{align}$$

Repare que
$$\begin{align}
10\%+8\%+7.5\%+13\% = 4\cdot9.625\% = 38.5\%
\end{align}$$

Então a média aritmética de um conjunto de dados com $n$ elementos é o único número que, repetido $n$ vezes, nos leva ao mesmo valor que obteríamos somando todos os elementos desse conjunto.

Se eu investi $\$100$ em $t=0$ nesse ativo financeiro, o valor do meu capital em $t=4$ é
$$\begin{align}
\$100(1+10\%)(1+8\%)(1+7.5\%)(1+13\%)=\$138.5.
\end{align}$$

Quando dizemos taxa de juros *média* aqui, nos referimos à única taxa de juros constante, que quando aplicada quatro vezes sobre $\$100$ nos levaria ao mesmo valor futuro $\$138.5$.

Se tentarmos aplicar a média das taxas, $9.6\%$, quatro vezes, obtemos
$$\begin{align}
\$100(1+9.6\%)^4=\$144.42,
\end{align}$$
que é diferente do valor futuro $\$138.5$, obtido ao compor as taxas individuais.

O que queremos é
$$\begin{align}
\$100(1+\tilde r)^4&=\$100(1+10\%)(1+8\%)(1+7.5\%)(1+13\%),\\
(1+\tilde r)^4 &= 1.385, \\
1+\tilde r &= (1.385)^{1/4}, \\
\tilde r &= (1.385)^{1/4}-1 = 8.483\%.
\end{align}$$

O que efetivamente fizemos foi calcular a taxa de juros total acumulada nos quatro anos, depois anualizamos tal taxa.

::: {.callout-note title="Taxa média de juros"}
Seja $r_1, r_2, \dots, r_n$ uma sequência de taxas de juro com $n$ elementos. A taxa média $\tilde r$ é a única taxa constante, que composta $n$ vezes, leva ao mesmo valor futuro que seria obtido componto $r_1, r_2, \dots, r_n$.
$$\begin{align}
  \tilde r = \left[\prod_{t=1}^n(1+r_t)\right]^{1/n}-1.
\end{align}$$
:::
...
Por exemplo, se um pacote de café no Supermercado Azul custava $P_0=\$8$ há um ano, e hoje custa $P_1=\$11$, então a (taxa de) inflação $\pi$ do café, entre um ano atrás e hoje, foi de
...
$$\begin{align}
  1 + \mu &= \frac{K_1}{K_0} = \frac{W_1/P_1}{W_0/P_0},\\
  &=\frac{W_0(1+r)/P_1}{W_0/P_0},\\
  &=(1+r)\frac{P_0}{P_1},
\end{align}$$
...
$$\begin{align}
  1+\mu = \frac{1+r}{1+\pi}.
\end{align}$${#eq-retreal}
:::
É importante ressaltar que ambas a taxa de retorno nominal $r$ e inflação $\pi$ precisam estar expressas na mesma frequência de incidência (mensal, anual, etc.).

#### Retorno real *ex-post* e *ex-ante* {.unnumbered}

No @exm-inflacao-1 nós calculamos o retorno da aplicação financeira em termos da quantidade de sementes, mas isso só foi possível depois de passado o tempo e observarmos as mudanças nos preços relevantes. 

A [@eq-retreal] é uma tautologia contábil que define a taxa real realizada $\mu$. Na realidade a decisão do agricultor entre investir no ativo financeiro ou estocar as sementes depende do palpite dele sobre o preço das sementes ao final do ano. 

O que importa para a formação das decisões de investimento dos agentes não é o retorno real *ex-post*, calculado depois do ocorrido, mas a expectativa do retorno real *ex-ante*:

$$\begin{align*}
  1 + E_t[\mu_{t+1}] = \frac{1+r}{1+E_t[\pi_{t+1}]}.
\end{align*}$${#eq-retreal-exante}
...
$$\begin{align}
  I = `r Pesos[1]` A + `r Pesos[2]` F + `r Pesos[3]` L + `r Pesos[4]` C + `r Pesos[5]` G.
\end{align}$$
...
Nos meses subsequentes, temos $I_2 = \$`r Indice[2]`$, e $I_3 = \$`r Indice[3]`$.

Os valores $I_t$ não são preços de um bem específico, e em muitas situações estamos mais interessados na variação percentual do que no valor absoluto em si.
$$\begin{align}
  I^*_t = \frac{I_t}{I_1}\cdot100
\end{align}$$

```{r, echo=FALSE}
df2 = data.frame(1:length(Arroz), Indice, IndiceP)
colnames(df2)=c("Mês", "Índice (\\$)", "Índice Padronizado")
knitr::kable(df2)
```
...
$$\begin{align}
    PV(\{FC_t\}^T_{t=0},r) &= \sum^T_{t=0} \frac{FC_t}{(1+r)^t},\\
...
$$\begin{align*} VP(\{FC_1,FC_2\},r) &= VP(\{FC_1\},r,1) + VP(\{FC_2\},r,2) \\
...
::: {.callout-tip title='@exm-seq-fcs-4: Tabela *price* de amortização de empréstimos' collapse='true'}
::: {#exm-seq-fcs-4}

Suponha que você tomou emprestado $\$25000$ pelo período de dez anos, a uma taxa de juros de $8\%$ ao ano. O empréstimo será quitado em dez parcelas iguais, de modo que ao pagamento da décima parcela, seu saldo devedor é zero.

  a. Qual o valor da parcela? 
  a. Quanto de cada parcela se refere a amortização do saldo devedor e quanto de cada parcela se refere ao pagamento de juros?
...
O Tesouro disponibiliza uma [calculadora](https://www.tesourodireto.com.br/titulos/calculadora.htm) que facilita esses cálculos. No dia a dia temos que lidar com detalhes de dias úteis no caledário civil e incidência de impostos sobre recebimentos de cupons e valor de face.
:::
:::
...
Infelizmente, a TIR só é bem definida (e portanto tem as interpretações que discutimos) se o fluxo de caixa em questão for "convencional". Uma sequência de fluxos de caixa é dita convencional se houver apenas uma mudança no sinal dos fluxos de caixa.

::: {.callout-tip title="@exm-tir-4 TIR de um fluxo de caixa não convencional" appearance="minimal" collapse="true"}
::: {#exm-tir-4}
...
::: {.callout-note title="Unicidade da taxa interna de retorno" collapse=true}
...
As duas condições abaixo garantem que existe apenas uma solução real $r^*$ (vão existir raízes complexas, que vamos ignorar), e que essa solução real é positiva.
...
Essa condição garante que existe apenas uma solução real. Na grande maioria das nossas aplicações essa mudança ocorre do primeiro para o segundo fluxo de caixa. Isto é, $FC_0<0$ e $FC_t>0$, $t=1,\dots,n$; ou $FC_0>0$ e $FC_t<0$, $t=1,\dots,n$.
  ...
  2. Assumindo que a mundança de sinal acontece entre $t=0$ e $t=1$, precisamos que o valor bruto (não descontado) dos fluxos de caixa acumulados entre $t=1$ e $t=n$ sejam do que $FC_0$, em módulo. Ou seja,
...
::: {.callout-tip title="@exm-tir-3 Cálculo da TIR no usando a linguagem ***R***" appearance="minimal" collapse="true"}
::: {#exm-tir-3}
```{r, echo=FALSE}
rm(list=ls())
```

Tome a sequência de fluxos de caixa descrita na tabela abaixo.
...
Para calcular a TIR nós precisamos resolver a equação [@eq-tir] numericamente. Vamos usar a função `polyroot()`, que obtém as raízes reais e complexas de uma equação polinomial.

```{r}
SFC = c(-100, 10, 10, 20, 30, 45) # <1>
```

1. Construímos um vetor cujas entradas são os fluxos de caixa (os coeficientes da equação polinomial), em ordem crescente de tempo.

```{r}
raizes = polyroot(SFC) # <2>
raizes
```

2. A função `polyroot()` retorna um vetor com as todas as raízes, reaiz e complexas, do polinômio.

```{r}
idx = abs(Im(raizes))<10^(-6) # <3>
raiz_real = Re(raizes[idx]) # <4>
raiz_real
```

3. Para nós só é relevante a raíz real. Então identificamos a entrada do vetor `raizes` cuja parte imaginária (usando a função `Im()`) é, em termos absolutos, muito pequena.

4. Usando a função `Re()`, ficamos apenas com a parte real do número, e transformamos o tipo da variável.

Lembre que a relação da TIR com a raíz do polinômio é tal que

$$ x = \frac{1}{1+r^*}, \qquad r^* = \frac{1}{x}-1. $$
```{r}
r_tir = 1/raiz_real-1 # <5>
r_tir
```

5. Então transformamos a raíz para obter a TIR.

Podemos criar uma função que repete os passos anteriores.

```{r}
calcula_tir = function(SFC){
  raizes = polyroot(SFC)
  idx = abs(Im(raizes))<10e-6
  if (sum(idx)!=1){
    return("Fluxo de caixa não convencional, múltiplas TIR.")
    }
  raiz_real = Re(raizes[idx])
  r_tir = 1/raiz_real-1
  return(r_tir)
}
```
...
Um mesmo saldo inicial de $S_0=\$1000$ é capitalizado ambos pelo regime composto e simples, com a mesma taxa de juros de $10\%$ ao ano. Quantos períodos são necessários para que o valor desse saldo capitalizado pelo regime composto exceda duas vezes o valor capitalizado pelo regime simples? (Dica: resolva numericamente simulando valores.)
:::
...
Para responder às perguntas abaixo, considere as convenções
  ...
  * 360 dias em 1 ano (dias corridos),
  * 52 semanas em 1 ano,
